...
//       System.out.println("\n INICIALIZACION DEL BUROCRATA");
        ...
        espectros.add(0xFFdddd); espectros.add(0xccFFcc);
    ...
    public void execute(){
         ...
         * Hay dos situaciones posibles al enviar el cancel.
         *  AGREE:
         ...
         *  Para ello:
         *   se cierra la sesión anterior y se toman las dimensiones
         *   o
         *   se inicia una subscripción y posteriormente un cancel.
         ...
         *    reciba el conversationID y rechazando todos los demás mensajes.
         *    Lo que conlleva que los vehículos entren en otro bucle del cual
         *    no saldrán mientras se les rechace;
         ...
         *   @Reflexión ¿Cómo evitar recibir el mensaje antes de tiempo
         *   y por ende evitar el interbloqueo?
         *   El mensaje se va a recibir si o si, no hay manera de sincronizarlos
         *   Lo que si se puede hacer es verificar que: "se está recibiendo lo
         *   que se espera recibir" y en otro caso enviar REFUSE.
         ...
         *   ello implica que solo se enviará el mensaje del conversationID
         *   cuando el burócrata termina la conversación con el controlador, 
         ...
         *  informados de sus capacidades.
         *  ¿Quédebe hace el burócrata?
         *  Hay dos opciones:
         ...
         * constructor aún no se han movido y por tanto no están CRASHEADOS,
        ...
        *  y decidir las estrategias a seguir, que son:
        *   1] Recibir la mayor información posible de cada vehículo.
        *       Lo que implica tener un control mínimo de cantidad de 
        *       información que éste aporta.
        *   2] Ayudar a un vehículo si éste pide ayuda,
        *       habiendo dos estrategias:
        *        2.1] El burócrata busca a otro vehículo para acercarse al 
        *              que está pidiendo ayuda.
        *             Con ello se presupone que el burócrata tendrá más
        *              mapa descubierto con lo que puede decidir hacia donde 
        *              dirigir al vehículo a auxiliar.
        *        2.2] El burócrata busca pero no encuentra a otro (vehículo) 
        *              por lo que dirigirá al vehiculo a auxiliar a una zona 
        *              inexplorada
        */
       int posicion;
       int estado_actual;
...
//           System.out.println(" ESTADO ACTUAL: "+ estado_actual);
           if(estado_actual == 0){
               ...
               */
                ...
                *   en caso contrario induzco al vehículo a que pare.
                *  ¿Cómo?
                *  @Opción1: Le envio un mensaje diciéndole que no se mueva y
                *   por ende queda a la espera de un nuevo mensaje.
                * 
                ...
                * parte de su situación y sería de interés para actualizar 
                * el mapa o no ya que en los estados distintos de 0] explorando,
                * no aporta información al mapa y no necesita el burócrata
                * deducirlo, pues ya está indicado en el estado del vehículo.
                */   
       ...
       try {
             ...
             *  debe realizo una imagen png para capturar la situación actual.
        ...
        *           ocasionaría un bloqueo en éste.
        *           ¿Cuando indicarle a dicho vehiculo que se puede mover?
        *            -] Cuando el burócrata decida enviarle coordenadas
        *            ¿Cuando será eso? Hay dos situaciones que se contemplan.
        *             Cuando NO hay vehículos disponibles para ayudar
        *              el burócrata de envia un mensaje con coordenadas
        *              de un lugar aún sin explorar.
        *             Cuando SI hayvehículos disponnibles para ayudar
        *              el burócrata enviará a uno para obtener más información
        *              sobre el mapa y poder así decidir a donde enviar el
        *              vehículo a auxiliar.
        *              Ese momento será cuando el vehículo auxiliador detecta en 
        *               su sensor la presencia del otro vehículo.
        *               por tanto debe ser en ese momento donde enviale las 
        *               nuevas coordenada para dirigirlo a algún lugar 
        *               inexplorado.
        *               ¿Como detectar eso? El vehículo auxiliador manda en su
        *               mensaje de percepciones las coordenadas del auxiliado,
        *               cuya clave será auxiliado_x, auxiliado_y
        *        - Enviarle una coordenadas al vehiculo seleccionado para ayudar
        *           Pero ¿Qué estado debería tener desde que empieza la ayuda
        *           hasta que llega?
        *           @RESPUESTA: El estado de activo pero ocupado.
        *           Y cuando llega al destino
...
/**
 *  Versión en la que un vehículo que ha llegado a la meta se le considera 
 *   candidato a ayudar
         for(int i=0; i<VEHICULOS_MAX; i++){
             if(estados_vehiculos.get(i) >=5) 
                 inmovil++;
             if(estados_vehiculos.get(i) == 0 || estados_vehiculos.get(i) == 2 ) 
                 candidatos_para_ayudar.add(i);
             if(estados_vehiculos.get(i) == 4)
...
//        vehiculos_en_movimiento = vehiculos_activos - vehiculos_pidiendo_ayuda.size();
        ...
        // NO hay nadie pidiendo ayuda
         ...
         /**
          * @OPCION elegida
          * Previo recuento de vehiculos Inactivos permanentemente (Inmóviles).
          * TODOS = VEHICULOS_MAX - Inmóviles
          * 
          * ¿Hay vehiculo pidiendo ayuda?
          *  SI hay
          *     ==> ¿Hay vehículo disponible?
          *          SI hay
          *             ==> ¿Cual mando? --> El más cercano.
          *          NO hay
          *             ==> ¿Qué hago? ----> Dale coordenadas
          * NO hay
          *    ==> ¿Eres vehiculo activo?
          *          SI  
          *             ==> ¿Qué hago?  ----> Déjalo hacer lo que estén haciendo
          *          NO
          *             ==> ¿Te puedes reactivar?
          *                  SI ==> ¿Estás en la meta?
          *                          SI ==> ¿Estaís TODOS?
          ...
          * sin ese miembro y por tanto cuando se pregunta por ¿TODOS?,
          * a él no se le tiene en cuenta.
          * Es decir, se necesita un contador de impedidos o realizar un 
          * conteo previo de los que están en juego.
          * 
          * @Nota: Ahora se hace recuento de vehículos_activos
          * tanto los que ya estaban activos, como los que se han activado.
          * 
        ...
        * 
        * @Modificación 17-12-2018: Ya está realizada la funcionalidad
        * para que el burócrata envie los mensajes a los vehiculos
        * ahora toca enviar mensajes a los vehículos en estado de exploración
        * y a los que están en estado de ayuda pero se les ha enviado coordenadas
        * porque no hay auxiliadores.
        * 
        */
/** FIXMI
 * ¿Se podría hacer un recuento de vehículos en movimiento?
 * Asçi solo se enviarían mensajes a los que realmente los gestionaran
 */
       ...
       /**
        * Puede que alguno se haya estrellado.
        *  Por lo que no podrá pedir las percepciones.
        *  Por eso debo saber cuantos vehiculos_en_movimiento hay
        *  para adecuar la cantidad de mensajes que espero recibir.
        *  Al inicio de la siguiente iteración.
        */
        ...
        * Por tanto el burócrata entra en un estado de escucha interminable
 ...
 */
     ...
     *  es decir, un desplazamiento de 5 unidades en cada coordenada.
    ...
    /**
     * @author: Germán
     * Función auxiliar para obtener al vehículo más cercano del que 
     *  está pidiendo ayuda.
    ...
    /**
     * @author Germán
     * Función auxiliar que proporciona las coordenadas de una región cercana
     *  que tiene más píxeles sin descubrir
     * @param percepciones_de_vehiculo 
     * @return Devuelve un Json con la clave 'dirigete_a':'true'
     *          y las coordenadas 'x':x e 'y':y
     */
    ...
    /**
     * @author Germán
     * Método auxiliar que cuenta los pixeles que aún no se han descubiento al
     *  rededor de las coordenadas indicadas, con un rango de visión dado.
     * @param x
     * @param y
     * @param rango
